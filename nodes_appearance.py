...
class MarigoldIIDAppearance:
    ...
    RETURN_NAMES = ("albedo", "roughness", "metallicity")
    FUNCTION = "process"
    CATEGORY = "Marigold IID Appearance"

    DESCRIPTION = "Runs Marigold IID Appearance on input images and returns albedo/roughness/metallicity maps."
    ...
    def process(
        ...
        if kind is not None and kind != "appearance":
            raise RuntimeError(
                f"This node expects the *appearance* model, but got kind={kind!r}. Use `Load Marigold IID Appearance Model`."
            )
            ...
            with torch.inference_mode():
                with torch.autocast(mm.get_autocast_device(device), dtype=autocast_dtype) if autocast_condition else nullcontext():
                    for i in range(B):
                        ...
                        roughness_out.append(_pil_to_image_tensor(vis["roughness"]))
                        metallicity_out.append(_pil_to_image_tensor(vis["metallicity"]))
        ...
        roughness = torch.cat(roughness_out, dim=0).clamp(0, 1).cpu().float()
        metallicity = torch.cat(metallicity_out, dim=0).clamp(0, 1).cpu().float()

        if mask is not None:
            keep = _mask_to_keep(mask.detach().cpu().float(), (albedo.shape[1], albedo.shape[2]), albedo.shape[0]).unsqueeze(-1)
            albedo = albedo * keep
            roughness = roughness * keep
            metallicity = metallicity * keep

        return (albedo, roughness, metallicity)


class MarigoldIIDAppearanceExtended:
    @classmethod
    def INPUT_TYPES(cls):
        return MarigoldIIDAppearance.INPUT_TYPES()

    RETURN_TYPES = ("IMAGE", "IMAGE", "IMAGE", "IMAGE")
    RETURN_NAMES = ("albedo", "material", "roughness", "metallicity")
    FUNCTION = "process"
    CATEGORY = "Marigold IID Appearance"

    DESCRIPTION = "Runs Marigold IID Appearance and also returns the stacked `material` map."

    def process(self, iid_model, images, denoise_steps=4, processing_resolution=768, match_input_resolution=True, ensemble_size=1, mask: torch.Tensor | None = None):
        _require_module("diffusers", "pip install -r custom_nodes/Comfyui-marigold-intrinsics/requirements.txt")
            ...
            with torch.inference_mode():
                with torch.autocast(mm.get_autocast_device(device), dtype=autocast_dtype) if autocast_condition else nullcontext():
                    for i in range(B):
                        ...
                        albedo_out.append(_pil_to_image_tensor(vis["albedo"]))
                        material_out.append(_pil_to_image_tensor(vis["material"]))
                        roughness_out.append(_pil_to_image_tensor(vis["roughness"]))
                        metallicity_out.append(_pil_to_image_tensor(vis["metallicity"]))
        ...
        albedo = torch.cat(albedo_out, dim=0).clamp(0, 1).cpu().float()
        material = torch.cat(material_out, dim=0).clamp(0, 1).cpu().float()
        roughness = torch.cat(roughness_out, dim=0).clamp(0, 1).cpu().float()
        metallicity = torch.cat(metallicity_out, dim=0).clamp(0, 1).cpu().float()
        ...
        return (albedo, material, roughness, metallicity)
