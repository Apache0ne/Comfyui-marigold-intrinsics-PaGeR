...
class Attention(nn.Module):
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        if use_memory_efficient_attention_xformers:
            ...
            if is_custom_diffusion:
                ...
                processor = XFormersAttnProcessor(attention_op=attention_op, use_RoPE=self.use_RoPE)
        else:
            if is_custom_diffusion:
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        if len(hidden_states.shape) == 3:
            ...
            encoder_hidden_states = rearrange(encoder_hidden_states, '(b t) l c -> b (t l) c', t=6) if encoder_hidden_states is not None else None
            ...
            hidden_states = rearrange(hidden_states, 'b (t l) c -> (b t) l c', t=6)
            encoder_hidden_states = rearrange(encoder_hidden_states, 'b (t l) c -> (b t) l c', t=6) if encoder_hidden_states is not None else None
            res = rearrange(res, 'b (t l) c -> (b t) l c', t=6)
        elif len(hidden_states.shape) == 4:
            b, c, h, w = hidden_states.shape
            hidden_states = rearrange(hidden_states, '(b t) c h w -> b (h w t) c', t=6, h=h, w=w)
            if encoder_hidden_states is not None:
                b, c, h, w = encoder_hidden_states.shape
                encoder_hidden_states = rearrange(encoder_hidden_states, '(b t) c h w -> b (h w t) c', t=6, h=h, w=w)
            ...
            hidden_states = rearrange(hidden_states, ' b (h w t) c' ' -> (b t) c h w ', t=6, h=h, w=w)
            if encoder_hidden_states is not None:
                encoder_hidden_states = rearrange(encoder_hidden_states, ' b (h w t) c' ' -> (b t) c h w ', t=6, h=h, w=w)
            res = rearrange(res, 'b (h w t) c -> (b t) c h w ', t=6, h=h, w=w)
        else:
            res = self.processor(
                self,
                hidden_states,
    ...
    def norm_encoder_hidden_states(self, encoder_hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.norm_cross, nn.LayerNorm):
            # sync_norm:
            encoder_hidden_states = rearrange(encoder_hidden_states, '(b t) l c -> b (t l) c', t=6)
            ...
            # sync_norm:
            encoder_hidden_states = rearrange(encoder_hidden_states, '(b t) c l -> b c (t l)', t=6)
            encoder_hidden_states = self.norm_cross(encoder_hidden_states)
            encoder_hidden_states = rearrange(encoder_hidden_states, 'b c (t l) -> (b t) l c', t=6)
            encoder_hidden_states = encoder_hidden_states.transpose(1, 2)
        else:
            assert False
...
def _uniform_uv(hw, device, dtype):
    jj = torch.arange(3840, dtype=dtype, device=device)
    ii = torch.arange(2160, dtype=dtype, device=device)
    jj = jj + 0.5
    ii = ii + 0.5

    # ---- ERP-global UV in [-1, 1] over the full panorama, sampled onto cubemap ----
    # Normalize to [-1, 1] across the entire ERP image
    # U goes left->right; V goes top->bottom
    U = torch.pi*((jj / 3840) * 2.0 - 1.0)    # (W,)
    V = (torch.pi / 2) * ((ii / 2160) * 2.0 - 1.0)    # (H,)
    U, V = torch.meshgrid(U, V, indexing='xy')          # (H, W)

    erp = torch.stack([U, V], dim=-1).permute(2, 0, 1)            # (H, W, 2)

    # Sample this ERP-global UV into cubemap layout
    faces = erp_to_cubemap(erp, face_w=hw).permute(0, 2, 3, 1)   # list[6] of (2, hw, hw)                   # (6, 2, hw, hw)
    return faces[..., 0], faces[..., 1]  # (6, hw, hw), (6, hw, hw)

def _geom_freq(num_pairs: int, base: float, device, dtype):
    if num_pairs <= 0:
        return None
    i = torch.arange(num_pairs, device=device, dtype=dtype)
    # Classic RoFormer schedule; denominator uses num_pairs to keep magnitudes stable per-head
    return base ** (-2.0 * i / max(1, num_pairs))

def _rotate_pairs(t, cos, sin):
    # t: (BH, N, 2P); cos/sin: (1, 1, N, P)
    t1 = t[..., 0::2]
    t2 = t[..., 1::2]
    out1 = t1 * cos - t2 * sin
    out2 = t1 * sin + t2 * cos
...
def _apply_rope_2d(q, k, cos_t, sin_t, cos_p, sin_p):
    # q,k: (BH, N, D). Split D into θ-half and φ-half.
    BH, N, D = q.shape
    assert D >= 2, "head dim too small for RoPE"
    P = D // 2
    P_theta = P // 2
    P_phi   = P - P_theta
    D_theta, D_phi = 2*P_theta, 2*P_phi

    q_t, q_p, q_rest = q[..., :D_theta], q[..., D_theta:D_theta+D_phi], q[..., D_theta+D_phi:]
    k_t, k_p, k_rest = k[..., :D_theta], k[..., D_theta:D_theta+D_phi], k[..., D_theta+D_phi:]

    if P_theta > 0:
        q_t = _rotate_pairs(q_t, cos_t, sin_t)
        k_t = _rotate_pairs(k_t, cos_t, sin_t)
    if P_phi > 0:
        q_p = _rotate_pairs(q_p, cos_p, sin_p)
        k_p = _rotate_pairs(k_p, cos_p, sin_p)

    q = torch.cat([q_t, q_p, q_rest], dim=-1)
    k = torch.cat([k_t, k_p, k_rest], dim=-1)
    return q, k


class XFormersAttnProcessor:
    """
    xFormers attention processor with optional 2D RoPE for cubemap panoramas.
    - Caches θ/φ (or u/v) and their cos/sin banks per shape/device/dtype.
    - Switch between 'spherical' (θ,φ in radians) and 'uniform' (u,v in [-1,1]).
    """

    def __init__(
        self,
        attention_op: Optional[Callable] = None,
        use_RoPE: bool = False,     # 'spherical' | 'uniform'
        faces: int = 6,
        rope_base: float = 10000.0,
        apply_in_self: bool = True,
        apply_in_cross: bool = False,     # usually False for text cross-attn
    ):
        self.attention_op = attention_op
        ...
        self.apply_in_self = apply_in_self
        self.apply_in_cross = apply_in_cross

        # Lazy caches keyed by (device, dtype, faces, H, W, P_theta, P_phi, rope_base)
        self._pos_cache= {}
        self._cs_cache = {}

        # Last known spatial shape (when input arrived as 4D and we can infer H,W)
        self._last_hw: Optional[Tuple[int, int]] = None

    # ---- internals: caches ----

    def _get_positions(self, device, dtype, H, W):
        key = (device, dtype, self.faces, H, W)
        if key in self._pos_cache:
            return self._pos_cache[key]  # (N,), (N,)

        u, v = _uniform_uv(H, device, dtype)
        theta = u.reshape(-1)  # just "x-like" coordinate
        phi   = v.reshape(-1)  # "y-like" coordinate
        
        self._pos_cache[key] = (theta, phi)
        return theta, phi

    def _get_cos_sin(self, device, dtype, H, W, Dh):
        # Split head dim Dh into θ/φ halves like in _apply_rope_2d
        P = Dh // 2
        P_theta = P // 2
        P_phi   = P - P_theta

        key = (device, dtype, self.faces, H, W, P_theta, P_phi, self.rope_base)
        if key in self._cs_cache:
            return self._cs_cache[key]  # cos_t, sin_t, cos_p, sin_p

        theta, phi = self._get_positions(device, dtype, H, W)  # (N,), (N,)
        N = theta.numel()

        # frequencies
        w_t = _geom_freq(P_theta, self.rope_base, device, dtype)
        w_p = _geom_freq(P_phi,   self.rope_base, device, dtype)

        # angles -> (N,P)
        def cs(angle, w):
            if w is None or w.numel() == 0:
                # return trivial tensors with correct broadcastable shape
                return (None, None)
            ang = angle[:, None] * w[None, :]  # (N,P)
            cos = torch.cos(ang)[None, ...]  # (1,1,N,P) to broadcast over (BH, N, 2P)
            sin = torch.sin(ang)[None, ...]
            return cos, sin

        cos_t, sin_t = cs(theta, w_t)
        cos_p, sin_p = cs(phi,   w_p)

        self._cs_cache[key] = (cos_t, sin_t, cos_p, sin_p)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecate_msg = "The `scale` argument is deprecated and will be ignored. Pass it via `cross_attention_kwargs` on the pipeline."
            # (Optional) print/log once if you want.

        residual = hidden_states

        # spatial norm (some models expect (B, L, C); you already handled the 6-face concat elsewhere)
        if attn.spatial_norm is not None:
            if hidden_states.shape[0] != 1:
                hidden_states = rearrange(hidden_states, '(b t) l c -> b (t l) c', t=self.faces)
                hidden_states = attn.spatial_norm(hidden_states, temb)
                hidden_states = rearrange(hidden_states, 'b (t l) c -> (b t) l c', t=self.faces)
            else:
                hidden_states = attn.spatial_norm(hidden_states, temb)
        ...
        if attention_mask is not None:
            # Expand mask's singleton query_tokens dimension:
        ...
        # ---------------- RoPE (optional) ----------------
        if self.use_RoPE:
            # Only apply when we know the spatial layout and keys are image tokens.
            apply_rope = False
            if encoder_hidden_states is None or encoder_hidden_states is hidden_states:
                # self-attention
                apply_rope = self.apply_in_self
            else:
                # cross-attention (e.g., to text) -> usually skip
                apply_rope = self.apply_in_cross

            if apply_rope:
                # infer H,W; fallback to last seen if not 4D this time
                H = W = int(math.sqrt(query.shape[1] // self.faces))

                # token count expected from panorama
                N_expected = self.faces * H * W
                # Ensure query/key lengths match the expected panorama token count
                if key.shape[1] == N_expected and query.shape[1] == N_expected:
                    Dh = query.shape[-1]
                    with torch.no_grad():
                        cos_t, sin_t, cos_p, sin_p = self._get_cos_sin(
                            device=query.device, dtype=query.dtype, H=H, W=W, Dh=Dh
                        )
                    # Broadcast caches to current (BH, N, ·) automatically.
                    if cos_t is not None or cos_p is not None:
                        query, key = _apply_rope_2d(query, key, cos_t, sin_t, cos_p, sin_p)
                # else: silently skip (e.g., when doing pooled tokens or different sequence)
...
class AttnProcessor2_0:
    ...
    def __init__(
        ...
        # Lazy caches exactly like in XFormers version
        self._pos_cache = {}
        self._cs_cache = {}

        # Save last H,W if input is 4D
        self._last_hw = None

    # ---------------------------------------------------------
    # ---- RoPE helper utilities (copy from XFormers version) --
    # ---------------------------------------------------------
    ...
    def _get_positions(self, device, dtype, H, W):
        ...
        # (N,) coordinates in [-1, 1] like _uniform_uv()
        u, v = _uniform_uv(H, device, dtype)
        theta = u.reshape(-1)
        phi = v.reshape(-1)
    ...
    def _get_cos_sin(self, device, dtype, H, W, Dh):
        ...
        theta, phi = self._get_positions(device, dtype, H, W)
        N = theta.numel()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # --- Original code unchanged until Q,K,V are computed ---
        residual = hidden_states
        ...
        if attention_mask is not None:
            attention_mask = attn.prepare_attention_mask(attention_mask, L, B)
        ...
        if self.use_RoPE:
            ...
            if apply_rope:
                # Infer H,W
                if self._last_hw is not None:
                    H, W = self._last_hw
                else:
                    # fallback: assume square
                    H = W = int(math.sqrt(query.shape[2] // self.faces))

                N_expected = self.faces * H * W
                if query.shape[2] == N_expected and key.shape[2] == N_expected:
                    Dh = query.shape[-1]
                    with torch.no_grad():
                        cos_t, sin_t, cos_p, sin_p = self._get_cos_sin(
                            query.device, query.dtype, H, W, Dh
                        )
                    if cos_t is not None or cos_p is not None:
                        query, key = _apply_rope_2d(query, key, cos_t, sin_t, cos_p, sin_p)
